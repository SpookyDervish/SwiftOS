...
void amd_am79c973::Send(uint8_t *buffer, int size)
{
    ...
    /*
    Now you could just wait until the card picks up the package.
    Alternatively, you can also set the TDMD bit (0x08) in CSR0 (i.e. write 0x48 to CSR0 to keep the interrupts activated) in order to shorten this waiting time.
    */
    registerAddressPort.Write(0);
...
void amd_am79c973::Receive()
{
    ...
    {
        if (!(recvBufferDescr[currentRecvBuffer].flags & 0x40000000)                  // check Error bit (ERR)
            && (recvBufferDescr[currentRecvBuffer].flags & 0x03000000) == 0x03000000) // check Start of Packet (STP) and End of Packet (ENP) bits , see page 184 of amd documentation
        {
