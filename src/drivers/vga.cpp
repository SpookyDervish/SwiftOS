...
VideoGraphicsArray::VideoGraphicsArray()
    : miscPort(0x3c2), crtcIndexPort(0x3d4), crtcDataPort(0x3d5), sequencerIndexPort(0x3c4), sequencerDataPort(0x3c5),
      ...
      attributeControllerReadPort(0x3c1), attributeControllerWritePort(0x3c0), attributeControllerResetPort(0x3da)
{
}
...
void VideoGraphicsArray::PutPixel(int32_t x, int32_t y, uint8_t colorIndex)
{
    ...
    uint8_t *pixelAddress = _sbuffers[_sback] + 320 * y + x;
    *pixelAddress = colorIndex;
}

void VideoGraphicsArray::PutPixel(int32_t x, int32_t y, int32_t offset, uint8_t colorIndex)
{
    if (x < 0 || 320 <= x || y < 0 || 200 <= y)
        return;

    uint8_t *pixelAddress = _sbuffers[_sback] + 320 * y + x + offset;
    *pixelAddress = colorIndex;
}
...
void VideoGraphicsArray::DrawRectangleOutline(uint32_t x, uint32_t y, uint32_t w, uint32_t h, uint8_t r, uint8_t g,
                                              uint8_t b)
{
    for (int32_t Y = y; Y < y + h; Y++)
        PutPixel(x, Y, r, g, b);
    ...
    for (int32_t Y = y; Y < y + h; Y++)
        PutPixel(x + w, Y, r, g, b);
    for (int32_t X = x; X < x + w + 1; X++)
        PutPixel(X, y + h, r, g, b);
}

void VideoGraphicsArray::DrawCircle(uint32_t x, uint32_t y, uint32_t radius, uint8_t r, uint8_t g, uint8_t b)
{
...
void VideoGraphicsArray::DrawCircleQuadrant(uint32_t x, uint32_t y, uint32_t radius, uint8_t r, uint8_t g, uint8_t b,
                                            int32_t quadrant)
{
}

void VideoGraphicsArray::DrawChar(char c, uint32_t x, uint32_t y, uint8_t r, uint8_t g, uint8_t b, int32_t vscale,
                                  int32_t hscale)
{
    ...
    else
    {
        for (uint32_t i = 0; i < 8; i++)
        {
            ...
            for (uint32_t j = 0; j < 8; j++)
            {
                if ((row & (1 << j)) >> j)
                {
                    FillRectangle(x + j * hscale, y + i * vscale, hscale, vscale, r, g, b);
                }
            }
        }
    }
...
void VideoGraphicsArray::DrawText(const char *string, common::uint32_t x, common::uint32_t y, common::uint8_t r,
                                  common::uint8_t g, common::uint8_t b, common::int32_t vscale, common::int32_t hscale)
{
    int32_t xOffset = 0;
    int32_t step = 8 * hscale;

    while (*string != 0)
    {
        DrawChar(*string, x + xOffset, y, r, g, b, vscale, hscale);
        xOffset += step;
        string++;
    }
}

void VideoGraphicsArray::DrawLine(uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2, uint8_t r, uint8_t g, uint8_t b)
{
    // Slope of the line
    uint32_t m = (y2 - y1) / (x2 - x1);
}

void VideoGraphicsArray::DrawBitmapImage(uint32_t x, uint32_t y, uint32_t w, uint32_t h, uint8_t *data)
{
    ...
    for (unsigned i = 0; i < h && i + y < 200; i++)
    {
        for (unsigned j = 0; j < w && j + x < 320; j++)
        {
            ...
            uint32_t y = (y + i);
            uint32_t x = (320 * pixelSize) + (x + j) * pixelSize;
            PutPixel(x, y, data[bmpOffset + j * (bmpBpp / 8)]);
            PutPixel(x, y, 1, data[bmpOffset + j * (bmpBpp / 8) + 1]);
            PutPixel(x, y, 2, data[bmpOffset + j * (bmpBpp / 8) + 2]);
        }
        bmpOffset -= rowSize;
    }
}
